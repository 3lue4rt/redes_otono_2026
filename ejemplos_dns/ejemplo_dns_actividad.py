...
# Propuesto: Utilizando la IP que puede ver en pantalla, ahora consulte por uchile.cl, pero consultele al DNS de cl.
# compare esta respuesta con la respuesta obteniida al preguntar por uchile.cl al resolver "8.8.8.8".
# ¿Qué diferencias observa?
print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")

dnslib_reply_3 = send_dns_message("uchile.cl.", "190.124.27.10", 53)
print_dns_reply_elements(dnslib_reply_3)

print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")

dnslib_reply_4 = send_dns_message("uchile.cl.", "8.8.8.8", 53)
print_dns_reply_elements(dnslib_reply_4)
