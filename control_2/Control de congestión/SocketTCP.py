...
class SocketTCP:
    ...
    @staticmethod
    def divide_message(message: bytes, n) -> list[bytes]:
        return [message[i:i+n] for i in range(0, len(message), n)]
    
    def wrap_data_as_segment(self, data, seq) -> bytes:
        return self.create_segment(Segment(False, False, False, seq, data))
    
    def is_valid_ack_stop_and_wait(self, segment, answer):
        parsed_segment = self.parse_segment(segment)
        parsed_answer = self.parse_segment(answer)
        handshake1 = parsed_segment.is_handshake_1() and parsed_answer.is_handshake_2()
        handshake2 = parsed_segment.is_handshake_2() and parsed_answer.is_handshake_3()
        # ...



    def send_using_stop_and_wait(self, message: bytes) -> None:
        if self.seq == None or self.direccion_destino==None:
            debug("El socket no está conectado a un cliente, abortando")
            return
        ...
        while current_data is not None:
            self.socket_udp.sendto(current_segment, self.direccion_destino, timer_index=0)
            try:
                answer, address = self.socket_udp.recvfrom(PACKET_SIZE_MAX)
                if self.is_valid_ack_stop_and_wait(current_segment, answer):
                    self.socket_udp.stop_timer(timer_index=0) # solo hay un timer
